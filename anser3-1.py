...
def object_func(v,alpha,u,beta):
    ...
    theta = m.acos(vx_a/va)
    # 为阻力不为零，使用exp来保证A始终大于0，指数部分为|math.cos(theta-alpha)|，越小越好，（也即横截面积越小越好）
    A = D ** 2 * m.exp(abs(m.cos(theta - alpha)))
...
def grad_descend(v_init,alpha_init,u,beta,lr,n):
    for _ in range(n):
        dv = 2*v_init
        dalpha = 2*(u * m.cos(beta)*m.sin(alpha_init)-m.sin(beta)*m.cos(alpha_init))*abs(m.cos(alpha_init - beta)) + \
                 v_init**2*abs(m.cos(alpha_init - beta))*(-m.sin(alpha_init-beta))
        v = v_init - lr*dv
